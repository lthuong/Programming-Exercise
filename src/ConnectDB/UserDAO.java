...
//	}
	...
	public boolean checkLogin(String email, String password) {
		...
		try {
			preparedStmt = conn.prepareStatement("SELECT * FROM ProgEx.users WHERE user_email = ? AND user_password = ?");
	...
	public boolean checkEmailExisted(String email) {
		...
		try {
			conn = DBConnection.getConnection();
			preparedStmt = conn.prepareStatement("SELECT * FROM ProgEx.users WHERE user_email = ?");
			preparedStmt.setString(1, email);
			ResultSet rs = preparedStmt.executeQuery();
			if (rs.next()) {
				emailExisted = true;
	...
	public int insertUserRegistration(User user) {
		...
		try {
			conn = DBConnection.getConnection();
			preparedStmt = conn.prepareStatement("INSERT INTO ProgEx.users (user_email, user_firstName, user_lastName, user_password) VALUES(?,?,?,?)");
